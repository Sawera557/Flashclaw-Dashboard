...
@jwt_required()
def dashboard():
    """Full pipeline dashboard — owners with deal counts + latest deals."""
    try:
        return jsonify(get_open_deals_by_owner())
...
def ai_context():
    try:
        return jsonify({'context': build_context_summary()})
    except HubSpotError as e:
        return jsonify({'error': str(e)}), 502
