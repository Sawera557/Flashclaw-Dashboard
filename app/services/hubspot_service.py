...
def get_owner_name_map():
    """Return dict of owner_id -> name."""
    return {o['id']: o['name'] for o in get_owners()}


def fetch_open_deals(max_pages=20):
    """Fetch all open deals with pagination. Returns list of dicts."""
    all_deals = []
    after = None
...
def get_open_deals_by_owner():
    """Aggregate open deals grouped by owner. Returns dashboard data."""
    owners_map = get_owner_name_map()
    deals = fetch_open_deals(max_pages=20)

    from collections import defaultdict
    by_owner = defaultdict(list)
    for d in deals:
        by_owner[d.get('owner_id','unassigned')].append(d)
...
def get_deals_for_owner(owner_id, limit=20, include_closed=False):
    ...
    if not include_closed:
        filters.append({'propertyName': 'dealstage', 'operator': 'IN', 'values': OPEN_STAGES})
...
def search_owner_by_name(query):
    """Fuzzy search owners by name/email."""
    q = query.lower().strip()
    return [o for o in get_owners() if q in o['name'].lower() or q in o['email'].lower()]


def search_deals_by_owner_name(query, limit=10):
    """Find deals by fuzzy owner name match."""
    matched = search_owner_by_name(query)
    results = []
    for owner in matched[:5]:
        deals = get_deals_for_owner(owner['id'], limit=limit)
        if deals:
            results.append({'owner': owner, 'deals': deals})
    return results


def build_context_summary():
    """Text summary of HubSpot pipeline for AI injection."""
